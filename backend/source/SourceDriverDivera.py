...
class SourceDriverDivera(SourceDriver):
    ...
    def __init__(self, instanceName: str, settings: Settings, lastAlarmEvent: Optional[AlarmEvent] = None, mockAPI: Optional[DiveraMockAPI] = None) -> None:
        ...
        self.__apiURL = self.getSettingString("api_url", "https://www.divera247.com") # adjustable for test purposes #TODO: check with invalid URL for example !!!
        ...
        self.__showVehicleStatus = self.getSettingBoolean("show_vehicle_status", True) #TODO: show vehicle status in alarm view as well
        self.__showCrewResponses = self.getSettingBoolean("show_crew_responses", True) #TODO: show crew status in standby view as well
        ...
        self.__maxAlarmAge = self.getSettingInt("max_alarm_age", 300) # default to 5 minutes
        ...
        self.__startupTimestamp = datetime.datetime.now()
    ...
    def __retrieveAlarmEvent(self) -> Tuple[Optional[AlarmEvent], bool]: #pylint: disable=too-many-return-statements
        try:
            response = requests.get(self.__apiURL + "/api/last-alarm?accesskey=" + self.__apiKey, timeout=self.__timeout) #TODO: make sure the timeout applies to the entire transfer (compare internet/docu/PowerAlarm action) #TODO: make sure we handle exceptions here (compare PowerAlarm action) ?!?
        except Exception as e:
            self.error("failed to retrieve last alarm event", e, False)
            return None, False
        ...
        try:
            ...
            # handle old or delayed alarms
            creationTimestamp = datetime.datetime.fromtimestamp(data["ts_create"]) #TODO: use ts_update here ?!? only check for new alarms (and always do upates ?!?)
            if self.__maxAlarmAge > 0:
                nowTimestamp = datetime.datetime.now()
                timeDelta = nowTimestamp - creationTimestamp
                if timeDelta.total_seconds() > self.__maxAlarmAge:
                    self.dbgPrint(f"alarm event is older than margin (app_start={self.__startupTimestamp}, now={nowTimestamp}, alarm_creation={creationTimestamp}, time_delta={timeDelta.total_seconds()}s, margin={self.__maxAlarmAge}s)")
                    return None, True
            ...
            alarmEvent.sender = SourceDriverDivera.SENDER_DIVERA
            ...
            if description != "" and crewResponses != "":
                ...
                alarmEvent.comment = f"{description}{crewResponses}" #TODO: move crew responses to dedicated extension/flex line/field

            self.__lastAlarmEvent = alarmEvent
            return alarmEvent, True
        except Exception as e:
            self.error("failed to handle last alarm event", e, False)
    ...
    def __createSettingEventIfRequired(self, rawResult: str = "", statusLine: str = "", latestChange: int = 0) -> Optional[SettingEvent]:
        ...
        settingEvent.sender = SourceDriverDivera.SENDER_DIVERA
        settingEvent.flags = SettingEvent.FLAGS_VALID
        settingEvent.raw = rawResult

        settingEvent.key = "news" #TODO: use constants everywhere; allow storage in "header" instead (via a setting)
    ...
    def __retrieveVehicleStatus(self) -> Tuple[Optional[SettingEvent], bool]:
        try:
            response = requests.get(self.__apiURL + "/api/v2/pull/vehicle-status?accesskey=" + self.__apiKey, timeout=self.__timeout) #TODO: make sure the timeout applies to the entire transfer (compare internet/docu/PowerAlarm action) #TODO: make sure we handle exceptions here (compare PowerAlarm action) ?!?
        except Exception as e:
            self.error("failed to retrieve vehicle status", e, False)
        ...
        try:
            ...
            self.error("failed to handle vehicle status", e, False)
            settingEvent = self.__createSettingEventIfRequired() # make sure we send a SettingEvent to reset/hide the current vehicle status
            return settingEvent, False
