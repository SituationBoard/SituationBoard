...
class SourceDriverDivera(SourceDriver):
    ...
    def __retrieveAlarmEvent(self) -> Tuple[Optional[AlarmEvent], bool]: #pylint: disable=too-many-return-statements
        try:
            response = requests.get(self.__apiURL + "/api/last-alarm?accesskey=" + self.__apiKey, timeout=self.__timeout)
    ...
    def __retrieveVehicleStatus(self) -> Tuple[Optional[SettingEvent], bool]:
        try:
            response = requests.get(self.__apiURL + "/api/v2/pull/vehicle-status?accesskey=" + self.__apiKey, timeout=self.__timeout)
        except Exception as e:
