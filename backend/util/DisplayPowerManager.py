...
class DisplayPowerManager(Module):
    ...
    def scanDevices(self, cecDevice: str = "") -> str:
        _, output = self.executeCECCommand("scan", cecDevice, 10)
        return output

    def listCECDevices(self) -> str:
        shellCommand = ["cec-client", "--list-devices"]
        _, output = self.__executeCommand(shellCommand, "", 10)
        return output
