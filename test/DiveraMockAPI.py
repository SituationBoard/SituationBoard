...
class DiveraMockAPI:
    ...
    def run(self, host: str = DEFAULT_API_HOST, port: int = DEFAULT_API_PORT, interactive: bool = True) -> None:
        if interactive:
            ...
            # set initial values
            self.set_http_status_override(None)
            self.set_current_alarm(DiveraMockAPI.ALARM_NONE)
            self.set_current_vehicle_status(DiveraMockAPI.VEHICLE_STATUS_0)
    ...
    def set_current_alarm(self, alarm: str) -> None:
        with self.signalLock:
            if self.interactive:
                # update alarm timestamps in interactive mode
                try:
                    alarmData = json.loads(alarm)
                    if "data" in alarmData:
                        tsNow = datetime.datetime.now()
                        alarmData["data"]["date"] = tsNow.timestamp()
                        alarmData["data"]["ts_create"] = tsNow.timestamp()
                        alarmData["data"]["ts_update"] = tsNow.timestamp()
                        alarmData["data"]["ts_response"] = tsNow.timestamp()
                        alarm = json.dumps(alarmData)
                except Exception as e:
                    print(f"Updating alarm timestamps failed ({e})")
    ...
    def set_current_vehicle_status(self, status: str) -> None:
        with self.signalLock:
            if self.interactive:
                # update status timestamps in interactive mode
                try:
                    statusData = json.loads(status)
                    if "data" in statusData:
                        tsNow = datetime.datetime.now()
                        for i in range(len(statusData["data"])):
                            statusData["data"][i]["fmsstatus_ts"] = tsNow.timestamp()
                        status = json.dumps(statusData)
                except Exception as e:
                    print(f"Updating status timestamps failed ({e})")
